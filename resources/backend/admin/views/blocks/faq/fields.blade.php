        ...
        @foreach(config('app.locales') as $lang)
            <div class="tab-pane fade @if($lang == config('app.locale')) show active @endif" id="nav-tab-{{ $lang }}">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        {{ Form::text($lang . '[title]', isset($block) && $block->translate($lang) ? $block->translate($lang)->title : null, ['id'=>'pageTitle', 'class'=>'form-control','placeholder'=>__('backend.pages.title'), ($lang == config('app.locale')) ? 'data-parsley-required' : '']) }}
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-sm-2 col-form-label text-lg-right" for="pageContent">{{ __('backend.pages.content', [], $lang) }}:</label>
                    <div class="col-sm-10">
                        {{ Form::textarea($lang . '[content]', isset($block) && $block->translate($lang) ? $block->translate($lang)->content : null, ['class'=>'form-control summernote']) }}
                    </div>
                </div>
            </div>
        @endforeach
    ...
    <div class="form-group row m-b-15">
        ...
        <div class="col-md-10 col-sm-10">
            <div class="input-group">
                ...
                {!! Form::text('date', isset($block) ? \Carbon\Carbon::parse($block->created_at)->format('d.m.Y') : \Carbon\Carbon::now()->format('d.m.Y'), ['id'=>'datepicker-date', 'class'=>($errors->has('date')) ? 'form-control datepicker is-invalid' : 'form-control datepicker', 'data-parsley-required']) !!}
            </div>
        </div>
    </div>
    ...
    <div class="form-group row m-b-15 m-t-30">
        ...
        <div class="col-md-10 col-sm-10">
            @include('views.buttons.save', [
                        'back_link' => route('backend.blocks.faq.index'),
                        'destroy_link' => route('backend.blocks.faq.destroy', isset($block) ? $block : ''),
                        'permission' => 'pages',
                    ])
        </div>
    </div>
